"""
CPU-based sampling for 4-digit addition (fallback if GPU sampling crashes).
Slower but more stable for evaluation.
"""
import os
import random
import re
...
def extract_prediction(output, prompt):
    ...
    match = re.search(r"(\d{5})", after)  # 5-digit result
    if not match:
        return None
...
MAX_NEW_TOKENS = 5  # 5-digit result
EVAL_PER_CARRY = 20
SEED = 42
TEMPERATURE = 0.8
TOP_K = 200

print("AUTOMATED SAMPLE & VERIFY (4-DIGIT ADDITION - CPU FALLBACK)")
print("Using CPU for stable sampling (slower but reliable)")
...
try:
    ...
    print("\nRunning CPU-based evaluation (this will be slower)...")
    ...
    report_lines = []
    report_lines.append("4-DIGIT ARITHMETIC PROMPT EVAL REPORT (CPU)")
    ...
    with open("results/llm_output_cpu.txt", "w") as f:
        f.write("\n".join(report_lines) + "\n")
    ...
    print("\nSaved report to results/llm_output_cpu.txt")
