"""
Prepare a 100-digit addition dataset using a data-centric mix:
- 70% stratified random lengths
- 10% cascading carry cases
- 10% extreme length imbalance
- 10% boundary/zero-heavy cases

Examples still use fixed-width operands and reversed targets to remain
...
def cascading_carry_pair():
    # Force a long run of 9s on one side and complementary digits on the other
    # so carries chain across many low-order positions.
    ...
    # Add a small positive value so the carry chain actually triggers.
    b_tail = random.randint(1, 9)
    b = b_tail

    # Occasionally make b wider but keep low digits favorable for carry chain.
...
def extreme_imbalance_pair():
    ...
    # Bias toward 0/1 to test exact copying and low-order perturbations.
...
def boundary_zeros_pair():
    ...
    # Overflow boundary: 100-digit + 100-digit -> 101 digits with high probability.
    if mode < 0.4:
        a = random.randint(5 * 10**(NUM_DIGITS - 1), 10**NUM_DIGITS - 1)
        b = random.randint(5 * 10**(NUM_DIGITS - 1), 10**NUM_DIGITS - 1)
        return a, b

    # Zero-heavy numbers with sparse non-zero positions.
    if mode < 0.8:
    ...
    # Explicit tiny+huge boundary probes.
...
def scenario_counts(total_examples):
    ...
    # Distribute rounding remainder deterministically by weight rank.
...
def canonical_pair(a, b):
    # Treat a+b and b+a as the same arithmetic fact to reduce leakage risk.
    if a <= b:
        return a, b
    return b, a
...
# Strict leakage prevention check (exact example strings cannot overlap).
