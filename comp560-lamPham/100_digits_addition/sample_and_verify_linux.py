"""
Sample and verify 100-digit addition with reversed targets.
"""
import os
import sys
import random
...
from models.phi_wrapper import PhiAdapter, PhiAdapterConfig
...
def load_model(out_dir, device):
    ...
    model_family = model_args.get('model_family', 'nanogpt')

    if model_family == 'phi':
        phicfg = PhiAdapterConfig(
        ...
        phi_source = model_args.get('phi_model_source', 'Phi-3-mini-4k-instruct')
        model = PhiAdapter(config=phicfg, phi_source_dir=phi_source)
    else:
        allowed_gpt_args = [
            'n_layer',
            'n_head',
        ...
        gpt_model_args = {k: model_args[k] for k in allowed_gpt_args if k in model_args}
    ...
    return model, encode, decode, model_family


def sample_single(model, model_family, encode, decode, prompt, device, dtype):
        ...
        with ctx:
            if model_family == 'phi':
                ...
                y = model.model.generate(
                    ...
                    use_cache=False,
                    pad_token_id=getattr(model.model.config, 'eos_token_id', 0),
                )
            else:
...
def main():
    ...
    model, encode, decode, model_family = load_model(OUT_DIR, DEVICE)
    print(f"Loaded checkpoint family: {model_family}")
    ...
    for a, b, scenario in eval_cases:
        ...
        output = sample_single(model, model_family, encode, decode, prompt, DEVICE, DTYPE)
