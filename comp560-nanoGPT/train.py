...
def get_batch(split):
    ...
    nums_lines = (len(data) - block_size) //16
    ix = torch.randint(nums_lines, (batch_size,))*16
...
if os.path.exists(meta_path):
    ...
    if answer_only_loss and 'stoi' in meta:
        ...
        if eq_token_id is not None:
            print(f"answer_only_loss ENABLED: '=' token_id={eq_token_id}, '\\n' token_id={nl_token_id}")
            print(f"  -> Loss will only be computed on answer digits (after '=' until '\\n')")
        else:
            print("WARNING: answer_only_loss enabled but '=' not found in vocabulary. Falling back to full loss.")
            answer_only_loss = False
...
# --- [MỚI] HÀM QUẢN LÝ ĐÁNH GIÁ (LOSS & ACCURACY) ---
@torch.no_grad()
def estimate_loss():
    out = {}
    model.eval()
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            ...
            mask = (Y != -1) 
            # Bỏ qua những token đề bài, chỉ yêu cầu khớp phần đáp án
            match = (preds == Y) | ~mask 
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        # --- [MỚI] IN RA MÀN HÌNH THÔNG SỐ ACCURACY ---
        ...
        if wandb_log:
            wandb.log({
                ...
                "val/loss": losses['val'],
                "val/acc": losses['val_acc'], # Ghi log accuracy lên wandb
                "lr": lr,
                "mfu": running_mfu*100,
            })
        ...
        # --- [MỚI] LOGIC EARLY STOPPING MỚI DỰA TRÊN ACCURACY ---
        early_stop_acc = 0.99 # Ngưỡng dừng 99%
